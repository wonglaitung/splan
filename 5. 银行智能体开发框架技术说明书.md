...
```mermaid
flowchart TB
    REQ["用户请求"] --> REASON["推理 Reason"]
    REASON --> DECIDE["决定下一步行动"]
    DECIDE --> ACT["行动 Act"]
    ACT --> CALL["调用工具 / 生成回复"]
    CALL --> OBS["观察 Observe"]
    OBS --> GET["获取工具结果"]
...
```mermaid
flowchart TB
    L1["L1（试点期）<br/>被动工具 · 单点实验"] --> L2["L2（应用期）<br/>专业助手 · 场景赋能"]
    L2 --> L3["L3（流程重构期）<br/>流程协作者 · 嵌入核心流程"]
    L3 --> L4["L4（业务整合期）<br/>业务驱动者 · 主动创造价值"]
    L4 --> L5["L5（超智能原生体）<br/>生态组织者 · 跨机构协同"]
```
...
```mermaid
flowchart TD
    REQ["业务需求"] --> SKILL["Skill 文件定义<br/>无需编写底层代码"]
    SKILL --> AGENT["Agent 实例化"]
    AGENT --> RUN["运行"]
```
...
flowchart TB
    subgraph APP["业务应用层"]
        ...
        A2["信贷尽调<br/>Java"]
        A3["客服问答<br/>Python"]
        A4["财富管理领航系统<br/>Multi-Agent"]
        A1 ~~~ A2 ~~~ A3 ~~~ A4
    end
    APP --> SDK
    subgraph SDK["Harness SDK 核心层"]
        subgraph AH["AgentHarness（统一入口）"]
            AL["AgentLoop（ReAct 引擎）<br/>构建上下文 → 调用LLM → 执行工具 → 循环直到完成"]
        end
        subgraph MODS["核心组件"]
            direction LR
            M1["Skill 系统<br/>SkillLoader / SkillRegistry"]
            M2["Tool 系统<br/>ToolRegistry / ToolExecutor"]
            M3["Guardrails<br/>Layer 1/2 GuardrailHook"]
            M4["Memory 系统<br/>Session / Context"]
            M1 ~~~ M2 ~~~ M3 ~~~ M4
        end
    end
    SDK --> LLM
    subgraph LLM["LLM 接入层"]
        direction LR
...
```mermaid
flowchart TB
    BSP["BASE_SYSTEM_PROMPT<br/>极简，仅定义角色<br/>'你是一个专业助手'"]
    SKILL["skill.md（技能文件）<br/>YAML frontmatter：name / tools<br/># 角色定位 / ## 工作流程 / ## 判断标准"]
    ENH["增强的系统提示词<br/>'你是一个专业助手<br/>---<br/># 信贷尽调技能...'"]
    BSP -->|"+"| ENH
    SKILL -->|"+"| ENH
```
...
flowchart TB
    ...
    HOOK1 --> L1["Layer 1: PII 规则检测（&lt;1ms）<br/>手机号识别器 · 身份证识别器..."]
    L1 --> UP["UniversalPIIGuardrail<br/>自动语言检测 + 脱敏"]
    UP --> L2["Layer 2: LLM Judge（~100ms）<br/>ComplianceJudge：Prompt注入检测 · 越权意图检测 · 风险等级判定"]
    L2 --> DEC{"风险判定"}
    DEC -->|高风险| BLK["拦截"]
    DEC -->|低风险| PASS["放行"]
    PASS --> LLM["LLM"]
    LLM --> HOOK2["Hook: AFTER_LLM_CALL"]
    HOOK2 --> OUT["输出安全检测（可选）"]
```
...
```mermaid
flowchart TB
    subgraph SC["Spring Cloud Orchestration Layer"]
        ...
        subgraph AGENTS["Agent 服务"]
            direction LR
            A1["Agent-A Service<br/>@Service · AgentLoop<br/>Skill: X"]
            A2["Agent-B Service<br/>@Service · AgentLoop<br/>Skill: Y"]
            A3["Agent-C Service<br/>@Service · AgentLoop<br/>Skill: Z"]
            A1 ~~~ A2 ~~~ A3
        end
        AGENTS --> REDIS["Redis<br/>共享状态"]
    end
```
...
flowchart LR
    ...
    subgraph CTL["Controller Layer"]
        C["ChatController · MCPController<br/>SkillController · ...<br/>业务逻辑编排 · 事件分发 · 模型状态同步"]
    end
    subgraph SDK["Harness SDK"]
        S["AgentHarness（统一入口）· MCPManager<br/>SkillRegistry · MemorySystem<br/>Agent执行引擎 · 工具系统 · 会话与记忆管理"]
    end
    UI --> CTL --> SDK
```
...
```mermaid
flowchart TB
    A["添加 MCP 服务器<br/>名称：data-platform<br/>传输方式：sse<br/>URL：http://内网IP:8080/sse<br/><br/>工具列表（连接后自动发现）：<br/>✓ query_enterprise_data<br/>✓ fetch_financial_report<br/>✓ get_risk_indicators"]
```
...
flowchart TB
    subgraph ENV["银行内网环境"]
        subgraph DESK["员工桌面（Windows）"]
            H["Harness Client（EXE）<br/>配置文件：~/.harness/<br/>• settings.json：API配置（指向内网模型服务）<br/>• mcp.json：MCP服务器配置<br/>• skills/：技能库"]
        end
        subgraph GW["统一 AI 接入网关"]
            G["• 本地大模型推理服务<br/>• Guardrails 安全护栏<br/>• 审计日志"]
        end
        subgraph MCP["MCP Server 集群"]
            M["• data-platform-server（数据中台访问）<br/>• regulation-server（制度规范查询）<br/>• crm-server（客户关系管理）"]
        end
        DESK -->|HTTPS 内网| GW
        GW -->|MCP/SSE| MCP
    end
...
```mermaid
flowchart TB
    A["实时监控<br/>Token 使用：████████░░ 80%（16K/20K）<br/>迭代次数：5/10<br/>工具调用：12 次<br/>当前状态：执行 BashTool<br/><br/>事件日志：<br/>14:32:01 开始执行<br/>14:32:03 调用 ReadTool<br/>14:32:05 调用 EditTool"]
```
