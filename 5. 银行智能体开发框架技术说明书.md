...
```
┌──────────────────────────────┐   ┌──────────────────────────────┐
│ AI · 数字员工（执行岗）        │   │ 人 · 管理者（签核岗）          │
│ 代笔需求 → 拟定方案 → 主力编码 │   │ 业务确认 → 批准方案 → 审查理解 │
│        → 逐段讲解代码          │   │        → 提交签核             │
└──────────────────────────────┘   └──────────────────────────────┘
   每步均经人的确认 / 批准 / 签核，AI 无自行推进与提交权
```

**open_ide 文件锁协作流程：**

```
开发者在 IDE 手工/插件写码 ──→ 调 open_ide(file) 打开并锁定该文件
                                      ↓
                           Agent 不得 write/edit/apply_patch 覆盖（tool.execute.before 硬拦截）
                                      ↓
              开发者确认改完（"改完了/可以继续"）──→ unlock_file(developer_confirmed=true)
                                      ↓
                              Agent 重新读取最新文件内容后继续测试/审查/提交
```
...
```
会话级（开发者自检） ──→ 项目级（项目经理跟踪） ──→ 组织级（领导决策 · 预算）
```
...
│  事件日志                            │
│  14:32:01 开始执行                   │
│  14:32:03 调用 ReadTool              │
│  14:32:05 调用 EditTool              │
└─────────────────────────────────────┘
```
