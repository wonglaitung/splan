...
block-beta
    columns 1
    A["战略愿景层 (WHY)\n转型动因 · 战略目标 · 未来展望"]
    B["业务架构层 (WHAT)\n三大业务域重构 · 端到端流程重构 · 六大场景"]
    C["应用架构层 (HOW)\n智能体分层架构 · 协作模式 · 核心能力实现 · 典型场景"]
    D["技术架构层 (WITH WHAT)\n应用构建层 · 框架层 · AI平台层 · 数据层 · 基础设施层"]
    E["治理架构层 (GOVERNANCE)\n三层治理体系 · 模型风险管理 · 合规与伦理"]
    F["组织架构层 (WHO)\n关键成功因素 · 组织模式 · 角色配置 · 文化变革"]
    A --> B --> C --> D --> E --> F
```
---
...
```mermaid
flowchart TB
    subgraph 增长引擎["智能驱动的增长引擎"]
        direction TB
        subgraph 传统["传统"]
            T1["客户洞察<br/>静态标签·批量画像"]
            T2["营销触达<br/>批量推送·转化率低"]
            T3["服务交互<br/>菜单导航·人工客服"]
            T4["产品匹配<br/>规则配置·有限选择"]
        end
        subgraph AI["AI"]
            A1["客户洞察<br/>实时画像·行为预测"]
            A2["营销触达<br/>精准推荐·千人千面"]
            A3["服务交互<br/>对话即服务·智能助手"]
            A4["产品匹配<br/>智能匹配·动态组合"]
        end
        传统 -.升级.-> AI
    end
```
...
```mermaid
flowchart TB
    subgraph 风险体系["自主运行的风险体系"]
        direction TB
        subgraph 传统["传统"]
            T1["欺诈监测<br/>规则引擎·事后分析"]
            T2["信用评估<br/>历史数据·静态模型"]
            T3["合规审查<br/>人工审核·抽样检查"]
            T4["风险预警<br/>定期报告·被动响应"]
        end
        subgraph AI["AI"]
            A1["欺诈监测<br/>实时图计算·事中拦截"]
            A2["信用评估<br/>多源数据·动态评分"]
            A3["合规审查<br/>智能审查·全覆盖"]
            A4["风险预警<br/>实时监控·主动预警"]
        end
        传统 -.升级.-> AI
    end
```
...
```mermaid
flowchart TB
    subgraph 智能运营["人机协同的智能运营"]
        direction TB
        subgraph 传统["传统"]
            T1["文档处理<br/>人工录入·OCR识别"]
            T2["流程执行<br/>人工流转·系统记录"]
            T3["运维管理<br/>被动响应·事后修复"]
            T4["资源调度<br/>经验配置·静态规划"]
        end
        subgraph AI["AI"]
            A1["文档处理<br/>智能理解·自动分类"]
            A2["流程执行<br/>智能编排·自动执行"]
            A3["运维管理<br/>预测性维护·主动优化"]
            A4["资源调度<br/>智能调度·动态优化"]
        end
        传统 -.升级.-> AI
    end
```
...
| 阶段 | 传统模式 | | AI模式 |
|------|---------|---|--------|
| 需求发现 | 广告被动推送 | → | 行为预测，主动触发 |
| 产品选择 | 菜单浏览 | → | 个性化推荐匹配 |
| 申请办理 | 表单填写 | → | 自动填表，无感审批签约 |
| 使用服务 | 人工服务 | → | 智能服务，主动优化 |
| 问题处理 | 客服被动响应 | → | 预测性干预，问题预防 |
...
flowchart TB
    subgraph L1["编排层 (Orchestration)"]
        O1["任务调度器<br/>任务分解·智能体调度·结果整合"]
        O2["冲突处理器<br/>处理智能体间的冲突与优先级"]
        O3["状态管理器<br/>维护跨会话的上下文与状态"]
    end
    subgraph L2["领域智能体层 (Domain Agents)"]
        D1["客户交互<br/>对话助手·推荐引擎·财富管家"]
        D2["风险决策<br/>信用审批·欺诈检测·合规审查"]
        D3["运营执行<br/>文档处理·流程自动化·资源调度"]
    end
    subgraph L3["基础智能体层 (Foundation Agents)"]
        F1["数据理解智能体<br/>解析非结构化数据"]
        F2["知识检索智能体<br/>从知识库获取信息"]
        F3["推理决策智能体<br/>基于规则与模型推理"]
        F4["行动执行智能体<br/>调用API执行操作"]
    end
    L1 --> L2 --> L3
```
...
```mermaid
flowchart TB
    subgraph L1["应用层 (Business Applications)"]
        A1["智能客服 · 风险决策 · 运营自动化 · 理财经理Copilot · ..."]
    end
    subgraph L2["应用构建层 (Application Building)"]
        B1["智能体编排平台<br/>低代码可视化编排<br/>快速原型验证<br/>标准化场景交付"]
        B2["智能体开发框架<br/>代码级深度定制<br/>复杂多Agent协同<br/>企业系统集成"]
    end
    subgraph L3["智能体框架层 (Agent Framework)"]
        C0["编排引擎（协同中枢）<br/>任务分解 · 智能体路由 · 并行执行 · 结果聚合"]
        C1["智能体运行时<br/>ReAct循环 · 状态管理"]
        C2["记忆系统<br/>工作记忆 · 情景记忆"]
        C3["工具调用<br/>MCP协议 · 权限控制"]
        C4["人机协同关卡<br/>风险关卡"]
        C5["可观测性与审计平面<br/>决策追踪 · 审计日志 · 合规证据"]
    end
    subgraph L4["AI平台层 (AI Platform)"]
        D1["MLOps<br/>CI/CD流水线·自动化测试·部署编排"]
        D2["模型管理<br/>模型仓库·版本控制·模型服务"]
        D3["特征工程<br/>特征库·特征监控·特征计算"]
    end
    subgraph L5["数据层 (Data Layer)"]
        E1["实时数据<br/>事件流·CDC·消息队列"]
        E2["批处理数据<br/>数据湖·数据仓库·数据建模"]
        E3["外部数据<br/>第三方API·数据市场·合作伙伴"]
    end
    subgraph L6["基础设施层 (Infrastructure)"]
        F1["计算<br/>GPU集群·容器平台·无服务器"]
        F2["存储<br/>对象存储·向量数据库·图数据库"]
        F3["网络<br/>低延迟网络·专线连接·安全网关"]
    end
    L1 --> L2 --> L3 --> L4 --> L5 --> L6
```
...
```mermaid
flowchart TB
    subgraph A["编排平台工作流（总行私有云）"]
        A1["用户对话界面"]
        A2["知识库检索"]
        A3["报告模板渲染"]
    end
    subgraph B["智能体开发框架（本地算力卡）"]
        B1["信用评分计算"]
        B2["风险模型推理"]
        B3["核心系统API调用"]
    end
...
flowchart TB
    subgraph S2["第二阶段：智能体协调层（动态编排）"]
        P1["Supervisor Agent<br/>意图识别 → 任务分解 → 智能体路由<br/>仅在需要推理的决策点调用模型"]
    end
    subgraph S1["第一阶段：确定性流程编排（固定流程）"]
        P2["Workflow Engine<br/>Agent A → Agent B → Agent C（硬编码调用链）<br/>流程可审计、可追溯、便于传统应用接入"]
    end
    subgraph S0["Spring Cloud 基础设施"]
        P3["Gateway · Eureka · Config · Redis（共用）"]
    end
    S2 --> S1 --> S0
```
...
flowchart TB
    ...
    subgraph B["技能配置（差异化）"]
        B1["角色定义 (Role) · 目标设定 (Goal)<br/>行为约束 (System Prompt)<br/>可用工具 (Tools) · 记忆范围 (Memory Scope)"]
    end
    subgraph C["智能体运行时（统一底座）"]
        C1["ReAct循环：思考→行动→观察<br/>状态管理：会话状态·执行状态<br/>通信机制：消息总线·事件驱动<br/>工具调用：MCP协议·参数映射"]
    end
    A --> B --> C
```
...
```mermaid
flowchart TB
    subgraph infra["Spring Cloud 基础设施"]
        G["Gateway<br/>路由"]
        E["Eureka<br/>服务发现"]
        C["Config<br/>配置中心"]
    end
    A["Agent-A Service<br/>@Service"] --> B["Agent-B Service<br/>@Service"] --> D["Agent-C Service<br/>@Service"]
    B --> R["Redis<br/>共享状态"]
```
...
```mermaid
flowchart TB
    S["Supervisor Agent<br/>意图识别 · 任务分解<br/>智能体路由 · 结果整合 · 完成判断"]
...
```mermaid
flowchart TB
    subgraph A["总行私有云（共享算力池）"]
        A1["适用：智能体编排平台<br/>标准化AI应用<br/>知识问答、FAQ客服<br/>报告生成、合同质检"]
        A2["特点：按需申请、统一运维、无需自建"]
    end
    subgraph B["本地算力卡（专属算力池）"]
        B1["适用：智能体开发框架<br/>核心系统集成<br/>资产负债表核心业务<br/>多Agent协同、实时风控"]
        B2["特点：自行采购部署、独立运维、完全可控"]
    end
    A ---|"专线连接"| B
```
...
```mermaid
flowchart TB
    S["战略治理层 (Strategic Governance)<br/>AI治理委员会（CEO牵头）<br/>战略决策·资源分配·风险审批<br/>平衡创新与风险，制定AI伦理原则"]
    E["执行治理层 (Execution Governance)<br/>IT部门：技术标准·平台建设·人才培养<br/>风险管理部：模型风险管理·合规审查"]
    O["运营治理层 (Operational Governance)<br/>嵌入式治理团队<br/>模型监控·合规检查·自动化审计<br/>实时风险控制与应急响应"]
    S --> E --> O
```
...
flowchart TD
    ...
    D --> E["模型监控<br/>↓<br/>持续监控·性能监控<br/>漂移检测<br/>异常告警"]
```
...
flowchart TB
    subgraph T["传统模式"]
        TB1["业务部门<br/>产品经理 · 业务分析"] --> TB2["IT部门<br/>开发团队 · 运维团队"]
    end
    subgraph G["目标模式"]
        GB1["业务部门<br/>AI产品负责人 · 领域专家 · 嵌入式AI团队"] --> GB2["IT部门<br/>AI平台工程 · 数据科学 · MLOps专家"]
    end
    T -.演进.-> G
```
...
```mermaid
flowchart LR
    A["全员AI素养<br/>↓<br/>基础培训<br/>意识提升<br/>风险认知"] --> B["专业AI技能<br/>↓<br/>角色定制<br/>技能认证<br/>实践项目"]
    B --> C["领导AI转型<br/>↓<br/>战略规划<br/>变革领导<br/>生态构建"]
```
