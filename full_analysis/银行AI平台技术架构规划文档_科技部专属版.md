...
flowchart TB
    ...
    subgraph L2["应用构建层"]
        B1["方案1：Dify平台<br/>工作流编排·RAG Pipeline<br/>知识库管理·API开放能力<br/>适用：标准化应用·快速原型"]
        B2["方案2：Python/Java SDK<br/>AgentLoop引擎·技能系统<br/>工具系统·Guardrails集成<br/>适用：复杂应用·深度定制"]
        B3["方案3：桌面客户端<br/>技能管理·多会话支持·MCP集成<br/>适用：个人办公·研发提效"]
    end
    subgraph L3["安全入口层"]
        C0["Guardrails网关（统一接入）"]
        C1["Layer 1: PII规则检测<br/>手机号·身份证·银行卡·其他12类PII<br/>UniversalPIIGuardrail（自动语言检测+脱敏）"]
        C2["Layer 2: LLM Judge（可选）<br/>ComplianceJudge（意图检测）<br/>StreamInterceptor（Token级实时拦截）"]
        C3["IAM权限继承机制<br/>与行内IAM对接·权限标签注入·RAG检索过滤"]
        C0 --> C1 --> C2 --> C3
    end
    subgraph L4["基础设施层"]
        D1["推理引擎<br/>vLLM集群·模型管理·多模型支持"]
        D2["知识库<br/>向量DB·Milvus/Qdrant·ETL Pipeline"]
        D3["算力调度<br/>GPU池化·负载均衡·弹性伸缩"]
        D4["模型仓库<br/>DeepSeek·Qwen·GLM"]
        D5["监控告警<br/>性能监控·成本监控·异常告警"]
        D6["容器平台<br/>Docker/K8s·CI/CD流水线·灰度发布"]
    end
    L1 -->|"业务API调用"| L2
    L2 -->|"统一API调用"| L3
    L3 -->|"安全API调用"| L4
...
```mermaid
flowchart LR
    A["业务应用<br/>业务请求"] --> B["应用构建层<br/>编排/调用"] --> C["安全入口层<br/>安全检测"] --> D["基础设施层<br/>模型推理"]
    C --> C1["Layer1检测"]
    C --> C2["Layer2检测"]
    C --> C3["权限继承"]
```
...
```mermaid
flowchart LR
    A["基础设施层<br/>模型响应"] --> B["安全入口层<br/>过滤响应"] --> C["应用构建层<br/>编排响应"] --> D["业务应用层<br/>业务响应"]
    B --> B1["输出过滤"]
    B --> B2["脱敏还原"]
    B --> B3["审计记录"]
```
...
| 资源项 | 配置 | 数量 | 用途 | 预算 |
|-------|------|------|------|------|
| GPU服务器 | H200*8 | 1台 | 模型推理 | ¥120-160万 |
...
| 资源项 | 配置 | 数量 | 用途 | 预算 |
|-------|------|------|------|------|
| GPU服务器 | H200*8 | 2台 | 模型推理（扩容） | ¥240-320万 |
...
flowchart TB
    subgraph cluster["vLLM推理引擎集群"]
        N1["vLLM节点1<br/>H200*8<br/>DeepSeek-V3"]
        N2["vLLM节点2<br/>H200*8<br/>Qwen2.5-72B"]
        N3["vLLM节点N<br/>H200*8<br/>GLM-4-9B"]
...
flowchart TB
    subgraph net["银行内网"]
        Z1["DMZ区<br/>API网关 · 负载均衡"]
        Z2["应用区<br/>业务应用 · AI平台 · Guardrails"]
        Z3["数据区<br/>数据库 · 向量DB · 模型仓库"]
        Z4["算力区<br/>GPU集群 · 推理引擎"]
        Z5["管理区<br/>运维管理 · 日志审计"]
        Z6["监控区<br/>监控系统 · 告警系统"]
    end
```
...
flowchart TB
    subgraph store["数据存储层"]
        S1["结构化数据<br/>PostgreSQL · 业务数据"]
        S2["非结构化数据<br/>对象存储(MinIO) · 文档/图片/音频"]
        S3["向量数据<br/>Qdrant/Milvus · 知识库向量"]
        S4["缓存数据<br/>Redis Cluster · 热点数据缓存"]
        S5["数据管道（ETL）<br/>数据采集 → 清洗 → 转换 → 向量化 → 存储"]
    end
```
...
flowchart TD
    ...
    D --> E["向量存储<br/>↓<br/>Qdrant存储·分片"]
    E --> F["更新索引<br/>↓<br/>定时任务·增量更新·全量重建"]
```
...
flowchart TB
    subgraph docker["Docker部署架构"]
        C1["容器1<br/>Guardrails网关<br/>Port: 8000"]
        C2["容器2<br/>Dify平台·应用构建<br/>Port: 3000"]
        C3["容器3<br/>vLLM推理引擎<br/>Port: 8001"]
        C4["容器4<br/>Qdrant向量DB<br/>Port: 6333"]
        C5["容器5<br/>PostgreSQL数据库<br/>Port: 5432"]
        C6["容器6<br/>Redis缓存<br/>Port: 6379"]
        DC["Docker Compose统一管理<br/>服务依赖管理 · 网络配置 · 存储卷管理"]
    end
```
...
flowchart TB
    subgraph gw["Guardrails网关架构"]
        H["HTTP Gateway（统一接入）<br/>FastAPI + Uvicorn（高性能异步）<br/>SSE流式响应支持 · 认证管理（config/client/both）"]
        subgraph proxy["ProxyHandler（代理核心）"]
            P1["Layer 1: PII规则检测<br/>Microsoft Presidio框架<br/>12+类PII识别器<br/>UniversalPIIGuardrail（自动语言检测）"]
            P2["Layer 2: LLM Judge（可选）<br/>Qwen3Guard-8B-Stream<br/>ComplianceJudge（意图检测）<br/>StreamInterceptor（Token级实时拦截）"]
            P3["IAM权限继承机制<br/>与行内IAM系统对接<br/>权限标签注入 · RAG检索过滤"]
            P4["SSEHandler（流式响应处理）<br/>Token级PII实时过滤 · 流式脱敏还原"]
        end
    end
    H --> proxy
```
...
flowchart LR
    subgraph iam["IAM权限继承架构"]
        A["业务应用<br/>用户请求 + Token"] --> B["Guardrails网关<br/>权限查询 · 权限注入"] --> C["IAM系统<br/>权限数据 · 角色信息"]
        R["权限缓存（Redis）<br/>TTL: 5分钟 · 缓存命中率 >90%"]
        B -.-> R
    end
```
...
flowchart TB
    ...
    N1["Guardrails节点1<br/>Active"]
    N2["Guardrails节点2<br/>Active"]
    N3["Guardrails节点3<br/>Standby"]
    MON["监控告警系统<br/>Prometheus + Grafana · 自动故障切换"]
    LB --> N1
    LB --> N2
    LB --> N3
    MON -.-> N1
...
flowchart TD
    ...
    Q --> Q2{"需要深度定制、复杂逻辑？"}
    Q --> Q3{"个人办公、研发提效？"}
    Q1 -->|是| R1["智能体编排平台（Dify）"]
    Q2 -->|是| R2["Harness SDK"]
    Q3 -->|是| R3["Harness Client"]
```
...
flowchart TB
    subgraph dify["Dify平台架构"]
        W["工作流编排引擎<br/>可视化拖拽设计<br/>节点：LLM·条件·工具·知识库<br/>变量传递·上下文管理"]
        R["RAG Pipeline<br/>文档上传 → 切片 → 向量化 → 存储<br/>检索：语义/关键词/混合检索<br/>重排序：提升检索精度"]
        A["API开放能力<br/>REST API · SSE流式响应<br/>与Guardrails网关集成"]
    end
```
...
flowchart TB
    subgraph sdk["Harness SDK架构"]
        A["AgentLoop引擎<br/>感知 → 思考 → 行动 → 观察 循环<br/>状态机管理 · 异常处理与重试"]
        S["技能系统<br/>技能定义（YAML/JSON）<br/>技能注册与管理 · 技能编排与组合"]
        T["工具系统<br/>内置工具（搜索·计算·数据库）<br/>自定义工具开发 · MCP工具集成"]
        G["Guardrails集成<br/>PII检测与脱敏 · 权限继承 · 输入输出过滤"]
    end
```
...
flowchart TB
    subgraph client["Harness Client架构"]
        C["桌面客户端（EXE）<br/>跨平台支持（Windows/Mac/Linux）<br/>技能可视化管理 · 多会话并发支持"]
        M["MCP集成<br/>Context7知识库连接<br/>工具扩展能力 · 自定义MCP服务器"]
        L["本地推理引擎<br/>支持本地模型部署<br/>离线使用能力 · 隐私保护"]
    end
```
...
```mermaid
flowchart TB
    CO["协调智能体（Coordinator）<br/>任务分解 · 智能体调度 · 结果聚合"]
    CO --> A1["分析智能体<br/>数据分析 · 特征提取"]
    CO --> A2["决策智能体<br/>风险决策 · 合规审查"]
    CO --> A3["执行智能体<br/>流程执行 · 结果反馈"]
```
...
flowchart LR
    A["现有应用"] -->|90%流量| A
    A --> B["新平台"]
    B -->|10%流量| B2["A/B测试验证<br/>用户反馈收集<br/>问题修复优化"]
```
...
flowchart LR
    A["现有应用"] -->|下线| X["✕"]
    A --> B["新平台"]
    B -->|100%流量| C["全面切换<br/>旧系统下线<br/>知识转移培训"]
```
...
```mermaid
flowchart LR
    subgraph L1["L1阶段（试点期）"]
        A["技术小组 2-3人<br/>架构师1人 · 工程师2人"]
    end
    subgraph L2["L2阶段（应用期）"]
        B["技术团队 5-8人<br/>架构师2人 · 工程师4人 · 运维2人"]
    end
    subgraph L3["L3阶段（流程重构期）"]
        C["AI中心 10-15人<br/>架构师2人 · 工程师6人<br/>运维3人 · 安全2人 · 数据2人"]
    end
    L1 --> L2 --> L3
```
...
flowchart LR
    subgraph collab["AI平台协作机制"]
        B["业务部门<br/>需求提出<br/>应用使用<br/>效果反馈"]
        A["AI团队<br/>需求评估<br/>应用交付<br/>持续优化"]
