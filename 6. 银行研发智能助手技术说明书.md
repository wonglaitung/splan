...
> **部署口径说明**：本说明书以 Claude Code 描述的能力为"能力口径"。我行实际操作可选兼容 MCP 的 AI 编程客户端（如 Claude Code、OpenCode、Cursor 等）接入，其中 **OpenCode** 是内网环境实际部署的首选客户端——详见第七章「AI 客户端接入」与第四章「研发智能助手全过程治理」。
...
| 组件 | 角色 | 回答的核心问题 | 对应章节 |
|------|------|---------------|---------|
| **Context7 知识库（MCP）** | 查得准 | AI 认不认识内部规范？会不会凭通用知识猜测 | 第五章（Context7 MCP 服务端） |
| **opencode-session-mgmt 三件套** | 管得住 | 流程走没走完？代码人能否接管？提效能不能证明 | 第四章（核心能力） |
| **AI 编程工具** | 用得来 | 内网用哪个客户端、怎么接入 | 第七章、第八章 |
...
**本章导读：** 本章只讲 Context7 的定位、接口契约与使用流程，供集成与测试参考。全部代码实现（切片算法、同步任务、部署脚本、Dockerfile）详见 **context7 项目仓库**（见 README「项目代码仓库」表，https://github.com/wonglaitung/context7）。
...
- **客户端接入配置**（OpenCode / Claude Code / Cursor）详见 **第七章「AI 客户端接入」**。

---

## 六、增量同步与容器化部署

**本章导读：** 只列运行形态与运维要点。完整的容器编排、`deploy.sh`、Dockerfile、配置参数均见 **context7 项目**（`sync_task`、`mcp_server`、`deploy/` 目录）。
...
> **提示：** 客户端接入前的插件与 MCP 配置不冲突，详见 7.1 / 7.5。

### 7.1 OpenCode 接入配置（内网实际部署客户端）
...
**通过流程治理插件启用 session-mgmt（详见第四章「研发智能助手全过程治理」）：**
...
### 7.5 AI 客户端行为引导（CLAUDE.md / AGENTS.md）
...
*文档版本：v2.1（Context7 章节精简为导读与接口契约，代码移至项目仓库）*
*最后更新：2026-08-09*
