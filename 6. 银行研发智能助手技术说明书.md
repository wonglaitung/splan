...
flowchart TB
    ...
    CTX -->|"="| BANK
    subgraph BANK["银行专用 Claude Code"]
        direction TB
        b0["Claude Code + Context7 = 银行研发智能助手"]
        b1["✅ 查询银行内部框架文档（实时）"]
        b2["✅ 生成符合行内规范的代码"]
        b3["✅ 理解老旧系统的历史架构"]
        b4["✅ 避免凭通用知识猜测内部框架行为"]
...
flowchart TB
    ...
    s2["2. Claude Code 调用 search_tech_manual(\"账户查询接口\")"]
    s3["3. 返回：核心系统接口规范.md - 账户查询接口 章节"]
    s4["4. Claude Code 判断：片段不够完整，需要全文"]
    s5["5. Claude Code 调用 get_manual_chapter(\"核心系统接口规范.md\")"]
    s6["6. 返回：完整接口规范文档"]
    s7["7. Claude Code 基于完整上下文生成代码"]
