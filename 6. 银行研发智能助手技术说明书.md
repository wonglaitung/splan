...
> **部署口径说明**：本说明书以 Claude Code 描述的能力为"能力口径"。我行实际操作可选兼容 MCP 的 AI 编程客户端（如 Claude Code、OpenCode、Cursor 等）接入，其中 **OpenCode** 是内网环境实际部署的首选客户端——详见第八章「AI 客户端集成」与新增章节「研发智能助手全过程治理」。
...
**通过流程治理插件启用 session-mgmt（详见第十一章「研发智能助手全过程治理」）：**
