...
```mermaid
flowchart TB
    subgraph TRAD["传统开发 · 刚性流水线（~10 个工作日）"]
        direction LR
        A1["需求"] --> A2["设计"] --> A3["编码"] --> A4["测试"] --> A5["修复"] --> A6["交付"]
    end
    subgraph AIDEV["AI 原生开发 · 两个环节（单功能 1 天量级）"]
        direction LR
        B1["AI 理解并执行<br/>（自然语言描述目标）"] --> B2["人类审核<br/>（确认意图与产出）"]
    end
    TRAD -->|"范式转变：写代码 → 描述需求"| AIDEV
```
...
```mermaid
flowchart TB
    L1["第一道坎 · 查得准<br/>查规范：search_tech_manual / get_manual_chapter"] --> L2["第二道坎 · 管得住<br/>走流程：插件工作流门禁"] --> L3["第三道坎 · 看得懂<br/>理解保障：comprehension + 设计推导"] --> L4["第四道坎 · 能量化<br/>度量：opencode-sm stats + CI 回写"]
```
...
flowchart TD
    F["文件"] --> M{"类型？"}
    M -->|".md"| A["按标题层级（#~######）切<br/>保住章节语义"]
    M -->|".py / .java / .go / .sql / .sh"| B["按函数 / 类 / 语句切<br/>保持代码块完整"]
    M -->|"PDF / DOCX"| C["段落 + Token 上限（默认 512）<br/>无明显结构时按语义分段"]
```
...
```mermaid
flowchart TB
    START["收到任务"] --> DEC{"涉及内部框架？"}
    DEC -->|"是"| S["search_tech_manual（关键词）"]
    S --> G["get_manual_chapter（取全文）"]
    G --> W["按规范写码"]
    DEC -->|"否"| OK["直接开发"]
```
...
```mermaid
flowchart LR
    subgraph SD["sdlc · 开发流程<br/>提交门禁启用（hasCommitGate=true）"]
        direction LR
        S1["需求"] --> S2["设计"] --> S3["编码"] --> S4["测试"] --> S5["审查"]
    end
    subgraph RD["reqdoc · 需求书工作流<br/>定稿无 git 门禁（hasCommitGate=false）"]
        direction LR
        R1["goal<br/>目标与场景"] --> R2["rules<br/>流程与规则"] --> R3["edge<br/>边界与异常"] --> R4["prd<br/>需求规格书"] --> R5["review<br/>业务确认"]
    end
```
...
```mermaid
flowchart TB
    subgraph iter["自由迭代区 · 需求/设计/编码/测试 可任意反复"]
        direction LR
        R["需求"] --> D["设计"] --> C["编码"] --> T["测试"]
    end
    T --> REV["审查<br/>（唯一 AI 不能自行推进的环节）"]
    REV -->|"全部阶段已审批"| GATE["git commit<br/>提交门禁硬拦截"]
    GATE -->|"尚有未通过阶段"| BLOCK["阻断 · commit_gate_check 查清单"]
    GATE -->|"全部已审批"| OK["放行入库"]
    REV -.->|"workflow_revisit：<br/>目标阶段 revision++ ·<br/>级联回退其后 approved 阶段"| iter
```
...
```mermaid
flowchart LR
    ADD["comprehension_add<br/>登记片段 + 自然语言 explain"] --> PEND["待审"]
    PEND --> CONF["comprehension_confirm<br/>确认（单次只认一个片段）"]
    PEND --> REJ["comprehension_reject + 意见<br/>附意见重写"]
    REJ --> ADD
    PEND --> RW["comprehension_rewrite<br/>重写（rewrites++）"]
    PEND --> MAN["comprehension_manual<br/>开发者自己改 / 删"]
    PEND --> ASK["comprehension_ask<br/>反问为什么 · 问答留痕"]
    CONF --> DONE["终态<br/>review_submit 要求全部落终态"]
```
...
```mermaid
flowchart TB
    DEV["开发者要手工改代码"] -->|"AI 先调 open_ide(file)"| LOCK["锁定文件<br/>写入 SQLite file_lock 表"]
    LOCK --> AIC["AI 可继续其它任务"]
    LOCK --> HARD["对被锁文件的<br/>write / edit / apply_patch<br/>服务端硬拦截"]
    HARD --> UL["开发者明确确认后<br/>unlock_file（多锁逐个确认）"]
    AIC --> UL
    UL --> RD["重新读取最新内容"]
    LOCK -->|"daemon 重启"| REC["锁自动恢复"]
```
...
**效能数据流：会话内 + CI 回写 + 基线三层汇入提效曲线**

```mermaid
flowchart LR
    subgraph SESS["会话内 · 插件自动记录"]
        A["Token / 费用 · 阶段耗时 · 需求迭代数<br/>编码-测试循环 · 一次通过率 · AI 净增行数"]
    end
    subgraph POST["合并后 · CI 回写"]
        B["返工率 · AI 参与模块增量覆盖率"]
    end
    subgraph BASE["基线"]
        C["workflow_baseline 预估工时<br/>（须开发者确认）"]
    end
    SESS --> E["opencode-sm stats<br/>（会话 / 项目 / 组 / 组织 四级）"]
    POST --> E
    C --> F["AI 提效百分比<br/>（预估工时 − 实际周期）/ 预估工时"]
    E --> F
    F --> G["研发提效曲线<br/>（规划书 35% 的实测参照）"]
```
...
```mermaid
flowchart LR
    A["写场景集<br/>（rule-based 判定，不用 LLM judge）"] --> B["冻结基线<br/>--variant baseline 快照入库"]
    B --> C["改一处规则 / 脚本"]
    C --> D["跑 --variant new<br/>--repeat N 聚合取通过率"]
    D --> E{"对比基线：有回退？"}
    E -->|"是"| F["归因失败场景<br/>规则措辞 / 判定口径 / 场景二义性"]
    F --> C
    E -->|"否"| G(["改动保留 · 沉淀资产"])
```
...
```mermaid
flowchart TB
    BIZ["业务：口述 + 丢材料<br/>现成资料进 01~04 需求资料目录"] --> SCAN["扫描通道 · reqdoc_scan<br/>解析 docx / pdf / xlsx / txt / md / json / csv"]
    BIZ --> ASK["提问通道 · 追问探针补缺"]
    SCAN --> FEAT["prd 阶段 · reqdoc_confirm_features<br/>拆功能点"]
    ASK --> FEAT
    FEAT --> DIRS["05_功能点 / 06_需求规格产出 子目录"]
    DIRS --> SCORE["reqdoc_score 五维打分<br/>total 服务端汇总 · business_confirmed 强制"]
    SCORE -->|"<60"| LOW["补主流程与异常边界"]
    SCORE -->|"60-84"| MID["补脱敏 / 权限 / 逆向撤销"]
    LOW --> ASK
    MID --> ASK
    SCORE -->|"≥85 停止追问"| RENDER["按《业务需求说明书》模板渲染<br/>逐字段标来源 [文档]/[问答]/[缺省] 绝不杜撰"]
    RENDER --> REVIEW["review_submit 业务确认"]
    REVIEW --> EXPORT["reqdoc_export 导出 Word 交付件"]
```

**质量飞轮（可持续改进）——应用开发人的开发期工具，业务需求提出人不碰它**：先分清两个使用方，否则极易混淆——**业务需求提出人**（使用方）在生产期用 reqdoc 工作流：放材料 → 被追问 → `reqdoc_score` 打分卡确认 → 拿 PRD，只接触打分卡（作为定稿门禁）；**应用开发人**（科技部研发团队，即本文读者）在**开发期**用飞轮 + 评测门（见 6.8）：对每个评测场景渲染出的 PRD 自动打五维分，做回归检测（哪一维掉了）与归因（哪个维度系统性薄弱 → 补对应支柱），据此迭代规则/探针/打分卡/模板/提示词。一句话，**飞轮用"需求书打几分"这个信号，去迭代"AI 代笔写需求书"这套应用**——度量落在需求书，改进落在应用行为面。打分卡是两方共用的桥梁：业务在生产期确认它、开发在开发期拿它当质量基线；追问探针（7 条，与五维一一映射）与渲染结构校验（对照模板 schema）保证"问到位 / 渲染不偏"，构成追问→打分→模板三支柱。**注意区分两个"质量"**：飞轮度量的是**需求书质量**（产出），迭代的是应用的**行为面能力**（规则/探针/打分卡/模板/工具描述——模型读进上下文的内容）；应用**机制面**的代码质量（工具逻辑/门禁/状态机/DB/CLI）不在此列，走 `bun test` + typecheck + CI（见 6.8.1）。这让"需求写得好不好"从主观评审变成可对比、可追踪的数字，是规划书"提效算得清"向需求侧的自然延伸。

**两个使用方与共用打分卡：**

```mermaid
flowchart LR
    subgraph DEV["开发期 · 应用开发人（科技部研发团队）"]
        D1["评测场景渲染 PRD"] --> D2["确定性评分器 打五维分"]
        D2 --> D3["回归检测 + 归因<br/>（哪一维掉了 / 哪维薄弱）"]
        D3 --> D4["改规则 / 探针 / 打分卡 / 模板"]
        D4 -.-> D1
    end
    subgraph PROD["生产期 · 业务需求提出人"]
        P1["放材料 + 口述"] --> P2["追问探针补缺"] --> P3["reqdoc_score 打分卡确认"] --> P4["拿 PRD"]
    end
    P3 -. "共用打分卡（单一事实源）" .- D2
```

**别当全自动优化：应用开发人机协同迭代，收敛有界**：飞轮的目的确实是"迭代出一套让模型产出高五维分需求书的提示词"，但它**不自动改提示词**——每次迭代都是「跑基线 → 看归因 → 改一处 → 再跑」，**在环里迭代的是应用开发人**（科技部研发团队），飞轮只指方向，与业务需求提出人无关。"合适"的范围由两个输入框死：**打分卡五维**（没覆盖的质量——业务洞察深度、创新性、可读性——飞轮看不见，也不优化）与**评测场景集**（提示词只对场景覆盖的需求形态泛化）。防过拟合靠**多模型复验**（本地弱模型是主要回归面）；**收敛判据** = 连续几轮分数稳定、回归检测无回退、场景全达标 → 停手重新冻结基线，别无限跑（场景集有限，后期只是在过拟合场景集本身）。一句话：**"跑飞轮 + 按归因人工改提示词 + 多模型复验"迭代出一套在五维与场景覆盖范围内适配多模型的提示词**——人机协同的迭代，不是全自动优化。

**改评分标准的操作流程（应用开发人改打分卡时走这条）**：打分卡改动有一个最容易踩的坑——**提示词是自动派生的**：`reqdocScoreRubric()` 把打分卡同源注入 reqdoc-r21 规则文本与 `reqdoc_score` 工具描述，所以你要改的是**打分卡源 + 评测镜像**，不是提示词文本本身。加扣分条件（A 类，最常见，加行内特定要求走这条）改 `REQDOC_SCORE_DIMS` 后，必须**同步 `score.ts` 的 `PREDICATES` 手工镜像**——condition 需逐字一致（含「」），且无测试强制同步，漏改时 `bun test` 全绿但评测测的是旧打分卡；新增维度（B 类大改）还牵动探针清单、模板字段映射、场景阈值，且各维 max 之和必须仍 = 100（`total = Σ dims` 硬契约）。收尾**重冻结基线**——打分卡变了旧基线失效。完整 8 步操作手册见 opencode-session-mgmt 的 `workflow-rules-eval` 技能（`.opencode/skills/workflow-rules-eval/SKILL.md`）。
...
```mermaid
flowchart TB
    CAUSE["不查文档 · 凭假设<br/>（三次返工根因）"] --> KNOW["Context7 知识侧<br/>编码前先查行内规范"]
    CAUSE --> PROC["全过程治理 过程侧<br/>设计推导留痕 · 重写轮次可量 · 提交门禁"]
    KNOW --> STOP["从个人偶然失误<br/>→ 流程上几乎不可能"]
    PROC --> STOP
```
...
*文档版本：v4.4（新增 13 处 mermaid 图：范式对比 / 能力分层 / 引导决策 / 双工作流与门禁回退 / 理解片段生命周期 / 手工修改锁定 / 效能数据流 / 六步闭环 / reqdoc 双通道 / 质量飞轮两使用方 / 切片决策 / 验证闭环）*
*最后更新：2026-08-16*
